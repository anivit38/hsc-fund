...
export default function ReviewQueue() {
  ...
  const sleeveOf = (id) => sleeves?.find((s) => s.id === id)?.name || '—';

  const queue = (pitches || [])
    .filter((p) => p.status === 'submitted' && p.analyst_id !== profile.user_id)
    .sort((a, b) => new Date(a.submitted_at) - new Date(b.submitted_at));

  return (
    <Layout title="Review Queue">
      <p className="muted" style={{ marginBottom: 16 }}>Every submitted pitch fund-wide, excluding your own — any PM can decide on any pitch (never their own).</p>
      <div className="card">
        <div className="table-wrap">
          <table>
            <thead><tr><th>Ticker</th><th>Kind</th><th>Sleeve</th><th>Analyst</th><th className="num">Conviction</th><th>Suggested wt.</th><th>Submitted</th><th>Expires</th><th></th></tr></thead>
            <tbody>
              {!loading && queue.map((p) => (
                <tr key={p.id}>
                  <td style={{ fontWeight: 700 }}>{p.ticker}</td>
                  <td><KindBadge kind={p.kind} /></td>
                  <td className="muted">{sleeveOf(p.sleeve_id)}</td>
              ...
              {!loading && !queue.length && <tr><td colSpan={9} className="empty">Nothing waiting on you right now.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
